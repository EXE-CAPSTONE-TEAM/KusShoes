...
DOCS: dict[str, tuple[str, str]] = {
    ...
    "GET /api/v1/auth/google": ("Bắt đầu đăng nhập Google", "Chuyển hướng (302) tới màn hình đồng ý của Google (OAuth 2.0, có `state` chống CSRF). App mobile truyền `client=mobile` kèm `code_challenge` (PKCE S256, bắt buộc). " + _P),
    "GET /api/v1/auth/google/callback": ("Callback đăng nhập Google", "Google gọi về sau khi đồng ý (`code`, `state`); tạo hoặc nối tài khoản rồi trả access token (`is_new_user`, `linked`) và đặt refresh token vào cookie. Phiên mobile thì chuyển về `MOBILE_GOOGLE_REDIRECT_URI?code=` (mã dùng một lần, 60 giây), không kèm token. " + _P),
    "POST /api/v1/auth/google/mobile/exchange": ("Đổi mã Google mobile lấy phiên", "App mobile gửi mã dùng một lần + `code_verifier` PKCE; trả access token và đặt refresh token vào cookie như `/auth/login`. Sai verifier cũng làm hủy mã; mã sai/đã dùng/hết hạn trả `AUTH_GOOGLE_MOBILE_CODE_INVALID`. " + _P),
    ...
    "POST /api/v1/admin/bake-jobs/{job_id}/requeue": ("Xếp lại bake job lỗi", "Chỉ với job `failed`; job trở về `awaiting_client` để KusStudio Desktop nhận lại. " + _AW),
    "POST /api/v1/admin/bake-jobs/{job_id}/cancel": ("Hủy bake job đang hoạt động", "Với job `awaiting_client` hoặc `claimed`; xoá file staging. " + _AW),
    ...
    "POST /api/v1/projects/{project_id}/bake": ("Tạo bake job cho KusStudio Desktop", "Cần model gốc `ready` (`EDITOR_MODEL_NOT_READY`), email đã xác thực, không trong ân hạn, còn hạn mức xuất; ghim phiên bản thiết kế. Trả job `awaiting_client` (202). Job chưa được nhận sẽ bị thay thế; job đang chạy trả `PROJ_BAKE_IN_PROGRESS`. " + _S),
    ...
    "POST /api/v1/projects/{project_id}/bake/{job_id}/retry": ("Thử lại bake job lỗi", "Chỉ với job `failed`; job trở về `awaiting_client`. " + _B),
    "POST /api/v1/projects/{project_id}/bake/{job_id}/cancel": ("Hủy bake job", "Với job `awaiting_client` hoặc `claimed`. " + _B),
    ...
    "POST /api/v1/editor/jobs/{job_id}/claim": ("Nhận job trên KusStudio Desktop", "Giao job cho một máy (lease `CLAIM_LEASE_SECONDS`); trả claim token và payload presigned cho sidecar. Máy khác đang giữ trả `JOB_ALREADY_CLAIMED`. " + _E),
    "POST /api/v1/editor/jobs/{job_id}/complete": ("Hoàn tất job", "Xác thực bằng header `X-Claim-Token`. Kiểm tra file staging trên R2 (tồn tại, dung lượng, chữ ký GLB/ZIP), copy sang key cuối, ghi export và trừ hạn mức đúng một lần. Gọi lại cùng token trả cùng kết quả."),
    "POST /api/v1/editor/jobs/{job_id}/fail": ("Báo job lỗi", "Xác thực bằng header `X-Claim-Token`; job chuyển `failed`, xoá file staging."),
