...
export function App({
  ...
  // Once authenticated (via login/register/demo or a restored token), leave the
  // /login or /register URL for /studio so the address bar matches the view.
  useEffect(() => {
  ...
  if (!user) {
    return (
      <AuthPage
        ...
        onModeChange={(nextMode) => onNavigate(nextMode === "register" ? "/register" : "/login")}
