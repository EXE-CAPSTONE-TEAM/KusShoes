...
import { adminAuditLogs } from '../../../api/adminClient';
...
export const AdminAuditLogs: React.FC = () => {
  ...
  const load = async (before?: string) => {
    if (!isAdmin) return;
    const result = await adminAuditLogs.list(session?.role ?? null, {
      q: search || undefined,
    ...
    setLogs(prev => (before ? [...prev, ...result] : result));
    setHasMore(result.length === 20);
  };

  useEffect(() => { load(); }, [search, targetType, action, isAdmin]);
    ...
    <div className={shared.page}>
      ...
      <div className={shared.toolbar}>
        <div className={`${shared.searchWrapper} glass-panel`}>
          ...
          <input
            ...
            placeholder="Tìm theo actor, target ID, payload..."
            value={search}
          ...
          <tbody>
            {logs.map(log => (
              <tr key={log.id}>
                ...
                <td>{log.actor_email} <StatusBadge status={log.actor_role} /></td>
                <td><StatusBadge status={log.target_type} tone="muted" label={log.target_type.replace('_', ' ')} /></td>
                <td><StatusBadge status={log.action} tone={ACTION_TONE(log.action)} label={log.action} /></td>
                <td className={shared.mutedCell}>{log.target_id.slice(0, 12)}...</td>
                <td className={shared.mutedCell} style={{ fontFamily: 'var(--font-mono)', fontSize: '0.76rem' }}>
                  {Object.keys(log.payload).length > 0 ? JSON.stringify(log.payload) : '—'}
                </td>
              </tr>
            ))}
            {logs.length === 0 && (
      ...
      {hasMore && (
        <div className={shared.loadMoreRow}>
          <button className={shared.textBtn} onClick={() => load(logs[logs.length - 1]?.created_at)}>Tải thêm</button>
        </div>
      )}
    </div>
  );
